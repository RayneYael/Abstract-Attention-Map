...
class LazySupervisedDataset(Dataset):
    ...
    def preprocess_qwen2vl(
        ...
        system_input = tokenizer.apply_chat_template(
        ...
        conv_user = source[0]
        conv_assistant = source[1]

        # regularize the conversation format
        user_role = conv_user["from"]
        user_content = conv_user["value"]
        ...
        if image_count > 0:
            ...
            conv = {
                ...
                "end_turn": conv_assistant.get("end_turn", True),
            ...
            for (point_x, point_y) in coord:
                try:
                    visual_token_index = get_token_index(
                        ...
                        [image_inputs[1]],  # 注意：这里应该是 sub_image 而不是 [image_inputs]
                        point_x,
                        point_y
                    )
                except Exception as e:
