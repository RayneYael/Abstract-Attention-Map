...
class VisionHead_MultiPatch(nn.Module):
    ...
    def forward(self,
                hidden_state_enc,  # shape: [n_enc, d_model]
                hidden_state_dec,  # shape: [n_dec, d_model]
                labels: Optional[torch.Tensor] = None,  # shape: [n_dec, n_enc]
...
class Qwen2_5_VLForConditionalGenerationWithPointer(Qwen2_5_VLForConditionalGeneration):
    ...
    def forward(self,
                ...
                verbose: bool = False, **kwargs) -> Union[Tuple, QwenVLwithVisionHeadOutputWithPast]:
        ...
        image_grid_thw_reshaped = image_grid_thw
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw_reshaped)
        ...
        if visual_token_indices_of_coordinates is not None:
            ...
            for i in range(batch_size):
                ...
                if original_image_grid_thw is not None and original_image_grid_thw.ndim == 3:
                ...
                if if_multi_patch:
                    if sample_labels is not None and sample_labels.shape[0] != target_indices.shape[0]:
