...
from gui_actor.utils import process_orginal_attention
...
def inference(conversation, model, tokenizer, data_processor, logits_processor=None, use_placeholder=False, topk=5):
    ...
    pred = {
        ...
        "original_attn_scores": None, # original attention scores
    }
    ...
    original_attn_scores = results.attentions[0] # tuple of [batch_size, num_heads, seq_len, seq_len]
    original_attn_scores = process_orginal_attention(original_attn_scores, model.config, input_ids) # torch tensor [batch_size, visual_token_num]
    pred["original_attn_scores"] = original_attn_scores.tolist()
