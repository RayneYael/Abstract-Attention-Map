...
from gui_actor.constants import chat_template, chat_template_subplot, grounding_system_message, grounding_system_message_subplot
...
def evaluate(model_name_or_path, model_type, use_subplot, data_fn, image1_dir, image2_dir, use_placeholder, topk, resize_to_pixels=None, subplot_resize_to_pixels=None):
    # initialize model
    ...
    for i, example in tqdm(enumerate(data), total=len(data)):
        if use_subplot:
            ...
            conversation = [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": grounding_system_message_subplot,
                        }
                    ]
                },
                {
            ...
            image_path = os.path.join(image1_dir, example["img_filename"])
        ...
        pred = inference(conversation, model, tokenizer, data_processor, use_subplot, logits_processor=logits_processor_pointer, use_placeholder=use_placeholder, topk=3)
        ...
        ele["attn_score"] = pred['attn_scores']
        ele["hit_topk_cord"] = topk_points
        ele["n_width_n_height"] = [pred['n_width'], pred['n_height']]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_subplot", action='store_false')
    ...
    image2_dir = os.path.join(args.data_path, "cropped_ScreenSpot-Pro/images") if use_subplot else None
    data_fn = os.path.join(args.data_path, "cropped_ScreenSpot-Pro/annotations/all.json") if use_subplot else os.path.join(args.data_path, "annotations/all.json")
    ...
    if os.path.exists(pred_path):
        ...
        results = evaluate(args.model_name_or_path, args.model_type, use_subplot, data_fn, image1_dir, image2_dir, args.use_placeholder, args.topk, resize_to_pixels, subplot_resize_to_pixels)
